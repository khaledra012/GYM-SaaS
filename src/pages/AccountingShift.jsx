...
const isLikelyShiftObject = (candidate) => {
    if (!candidate || typeof candidate !== 'object' || Array.isArray(candidate)) return false;
    return SHIFT_KEYS.some((key) => candidate[key] !== undefined && candidate[key] !== null);
};

const resolveCurrentShiftState = (rawPayload) => {
    if (!rawPayload) {
        return { shift: null, isOpen: false };
    }

    const hasOpenShiftFlag = rawPayload?.data?.hasOpenShift ?? rawPayload?.hasOpenShift;
    const possibleShiftPayloads = [
        rawPayload?.data?.currentShift,
        rawPayload?.currentShift,
        rawPayload?.data,
        rawPayload
    ].filter(Boolean);

    const shift = possibleShiftPayloads.find(isLikelyShiftObject) || null;
    const status = typeof shift?.status === 'string' ? shift.status.toLowerCase() : null;
    const isClosedByStatus = status === 'closed';
    const hasClosedTimestamp = Boolean(shift?.closedAt || shift?.closed_at);
    const hasOpenSignalInShift = Boolean(
        shift
        && !isClosedByStatus
        && !hasClosedTimestamp
        && (
            ...
            || shift.localDate
            || shift.displayNumber !== undefined
        )
    );

    const isOpen = hasOpenSignalInShift || (Boolean(hasOpenShiftFlag) && Boolean(shift));
...
const AccountingShift = () => {
    ...
    const fetchSummary = async () => {
        ...
        try {
            ...
            const [dashboardResult, debtsResult, currentShiftResult] = await Promise.allSettled([
                accountingAPI.getDashboardSummary(params),
                debtsAPI.getSummary(debtsSummaryParams),
                accountingAPI.getCurrentShift()
            ]);
            ...
            if (currentShiftResult.status === 'fulfilled') {
                const currentShiftState = resolveCurrentShiftState(currentShiftResult.value);
                setCurrentShift(currentShiftState.shift);
                setHasOpenShift(currentShiftState.isOpen);
            } else {
                console.error('Failed to fetch current shift:', currentShiftResult.reason);
    ...
    const formatAmountOrDash = (val) => (
        val === null || val === undefined || val === '' ? '---' : parseDecimal(val)
    );

    const formatTimeOrDash = (val) => {
        if (!val) return '---';
    ...
    const shiftPayload = currentShift;
    const hasRenderableOpenShift = hasOpenShift
        && isLikelyShiftObject(shiftPayload)
        && String(shiftPayload?.status || 'open').toLowerCase() !== 'closed'
        && !shiftPayload?.closedAt
        && !shiftPayload?.closed_at;
    const isShiftOpen = hasRenderableOpenShift;
    const canShowOpenShiftPanel = hasRenderableOpenShift;
    ...
    const hasDebtsSummary = Boolean(debtsSummary || summaryData?.debtsSummary);
    const expectedDrawerNow = canShowOpenShiftPanel
        && shiftPayload?.startingCash !== undefined
        && summaryData?.income !== undefined
        && summaryData?.expenses !== undefined
        ? parseDecimal(Number(shiftPayload.startingCash) + Number(summaryData.income || 0) - Number(summaryData.expenses || 0))
        : '---';
    ...
    return (
        <DashboardLayout>
            <div className="flex-between page-header mb-8">
                ...
                <div className={`status-badge ${isShiftOpen ? 'active' : 'inactive'}`} style={{ padding: '0.5rem 1rem', fontSize: '0.9rem' }}>
                    {isShiftOpen ? 'يوجد وردية مفتوحة' : 'لا يوجد وردية حالياً'}
                </div>
            </div>
                ...
                <>
                    ...
                    <div className="stats-grid">
                        <div className="stat-card">
                            ...
                            <p className="stat-value">{formatAmountOrDash(summaryData?.income)} <span style={{ fontSize: '1rem', color: 'var(--text-muted)' }}>EGP</span></p>
                        </div>
                        <div className="stat-card">
                            ...
                            <p className="stat-value" style={{ color: '#fca5a5' }}>{formatAmountOrDash(summaryData?.expenses)} <span style={{ fontSize: '1rem', color: 'var(--text-muted)' }}>EGP</span></p>
                        </div>
                        <div className="stat-card">
                            ...
                            <p className="stat-value" style={{ color: summaryData?.netProfit !== undefined && Number(summaryData?.netProfit) < 0 ? '#fca5a5' : '#38bdf8' }}>
                                {formatAmountOrDash(summaryData?.netProfit)} <span style={{ fontSize: '1rem', color: 'var(--text-muted)' }}>EGP</span>
                            </p>
                        </div>
                        <div className="stat-card" style={{ border: isShiftOpen ? '1px solid var(--accent-neon)' : '' }}>
                            ...
                            <p className="stat-value">
                                {summaryData?.currentDrawerCash !== null && summaryData?.currentDrawerCash !== undefined ? parseDecimal(summaryData.currentDrawerCash) : '---'} <span style={{ fontSize: '1rem', color: 'var(--text-muted)' }}>EGP</span>
                            </p>
                        </div>
                        <div className="stat-card">
                            ...
                            <p className="stat-value">{hasDebtsSummary ? parseDecimal(totalDebtPortfolio) : '---'} <span style={{ fontSize: '1rem', color: 'var(--text-muted)' }}>EGP</span></p>
                        </div>
                        <div className="stat-card">
                            ...
                            <p className="stat-value" style={{ color: '#fca5a5' }}>
                                {hasDebtsSummary ? parseDecimal(totalOutstandingDebt) : '---'} <span style={{ fontSize: '1rem', color: 'var(--text-muted)' }}>EGP</span>
                            </p>
                        </div>
                        <div className="stat-card">
                            <div className="stat-icon-wrapper" style={{ color: 'var(--accent-neon)', background: 'var(--accent-neon-light)' }}>
                            ...
                            <p className="stat-value">{hasDebtsSummary ? (visibleDebtsSummary.membersWithOutstandingDebtsCount || 0) : '---'}</p>
                        </div>
                        <div className="stat-card">
                            ...
                            <p className="stat-value">{hasDebtsSummary ? (visibleDebtsSummary.outstandingDebtsCount || 0) : '---'}</p>
                        </div>
                    </div>
                                ...
                                <div style={{ marginBottom: '2rem' }}>
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem' }}>
                                        <div className="detail-card">
                                            ...
                                            <p className="detail-value">{formatTimeOrDash(shiftPayload?.openedAt)}</p>
                                        </div>
                                        <div className="detail-card">
                                            <p className="detail-label">عهدة البداية</p>
                                            <p className="detail-value">{formatAmountOrDash(shiftPayload?.startingCash)} EGP</p>
                                        </div>
                                        <div className="detail-card">
                                            <p className="detail-label">Opened By</p>
                                            <p className="detail-value">{shiftPayload?.openedByName || '---'}</p>
                                        </div>
                                        <div className="detail-card">
